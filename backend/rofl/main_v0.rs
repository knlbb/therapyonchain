...
impl ExpertChatApp {
    ...
    async fn process_message_from_blockchain(self: Arc<Self>, message: String, user: String) -> Result<String> {
        ...
        println!("ChatGPT conversation history: {}", serde_json::to_string(&*conversation_history).unwrap());
        ...
        self.chat_history.add_assistant_message(user.clone(), assistant_reply.clone());

        println!("ChatGPT reply: {}", assistant_reply);
    ...
    async fn write_response_to_blockchain(self: Arc<Self>, env: Environment<Self>, response: String) -> Result<()> {
        ...
        let encoded_data = [
            ...
            ethabi::encode(&[Token::String(response)]),
        ]
        .concat();
...
//     let user_message = "Hello!".to_string();
